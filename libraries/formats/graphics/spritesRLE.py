...
def decodeRLESprites(data, romName, sectionIndex, mode):
    ...
    for entry in range(entryCount):
        ...
        while True: #There's no frame count, so brute forcing is required
            ...
            try:
                decompressed.save(os.getcwd()+f"/output/{romName}/sprites/RLE/{sectionIndex:06}/{entry:04}/{frameIndex:02}.png")
            except:
